...
describe("Cloudflare Worker entrypoint", () => {
  ...
  it("handles MCP initialize", async () => {
    ...
    expect(body.result?.serverInfo?.name).toBe("connectwise-automate-mcp");
  });
  ...
  it("lists all tools without credentials", async () => {
    ...
    expect(names).toContain("cwautomate_navigate");
    expect(names).toContain("cwautomate_status");
  ...
  it("returns a graceful error for a credential-requiring tool when unconfigured", async () => {
    const res = await mcp({
      ...
      id: 3,
      method: "tools/call",
      params: { name: "cwautomate_computers_list", arguments: {} },
    });
    expect(res.status).toBe(200);
    const body = (await res.json()) as {
